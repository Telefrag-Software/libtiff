 ...
 * TIFFReadPrivateDataSubDirectory(). This function is intended for use in
 * reading a private subdirectory from a TIFF file into a private structure. The
 * actual writing of data into the structure is handled by the setFieldFn(),
 * which is passed to TIFFReadPrivateDataSubDirectory() as a parameter. The idea
 * is to enable any application wishing to store private subdirectories to do so
...
int TIFFReadPrivateDataSubDirectory(TIFF *tif, toff_t pdir_offset,
                                                      ...
                                                      ...))
{
    ...
    for (fip = field_info, dp = dir, n = dircount; n > 0; n--, dp++)
    {
        if (tif->tif_flags & TIFF_SWAB)
        {
        ...
        if (dp->tdir_tag < fip->field_tag)
        {
            if (!diroutoforderwarning)
            {
                TIFFWarning(tif->tif_name,
                            "invalid TIFF private subdirectory; tags are not "
        ...
        {
            TIFFWarning(tif->tif_name,
                        "unknown field with tag %d (0x%x) in private "
...
static void EstimateStripByteCounts(TIFF *tif, TIFFDirEntry *dir,
                                    uint16_t dircount)
{
    register TIFFDirEntry *dp;
...
static int CheckDirCount(TIFF *tif, TIFFDirEntry *dir, uint32_t count)
{
    ...
    {
        TIFFWarning(tif->tif_name,
                    "incorrect count for field \"%s\" (%lu, expecting %lu); "
                    "tag ignored",
 ...
 */
static int cvtRational(TIFF *tif, TIFFDirEntry *dir, uint32_t num,
                       uint32_t denom, float *rv)
{
    if (denom == 0)
    {
        TIFFErrorExtR(tif, tif->tif_name,
...
static float TIFFFetchFloat(TIFF *tif, TIFFDirEntry *dir)
{
    ...
    uint32_t l =
        (uint32_t)TIFFExtractData(tif, dir->tdir_type, dir->tdir_offset);
    v = *(float *)&l;
    TIFFCvtIEEEFloatToNative(tif, 1, &v);
#endif
    return (v);
 ...
 */
static int TIFFFetchLongArray(TIFF *tif, TIFFDirEntry *dir, uint32_t *v)
{
    if (dir->tdir_count == 1)
    {
        v[0] = dir->tdir_offset;
...
/* The standard function TIFFFetchNormalTag() could definitely be replaced
   ...
   last argument. */
static int TIFFFetchNormalSubTag(TIFF *tif, TIFFDirEntry *dp,
                                 const TIFFFieldInfo *fip,
                                 int (*setFieldFn)(TIFF *tif, ttag_t tag, ...))
{
    ...
    { /* array of values */
        ...
        switch (dp->tdir_type)
        {
            ...
            case TIFF_SBYTE:
                ...
                cp = CheckMalloc(tif, dp->tdir_count * sizeof(uint16_t), mesg);
            ...
            case TIFF_LONG:
            case TIFF_SLONG:
                cp = CheckMalloc(tif, dp->tdir_count * sizeof(uint32_t), mesg);
        ...
        switch (dp->tdir_type)
        {
            ...
            case TIFF_SSHORT:
                 ...
                 */
                {
                    ...
                    {
                        uint16_t v = (uint16_t)TIFFExtractData(
                            tif, dp->tdir_type, dp->tdir_offset);
                        ok = (fip->field_passcount
            ...
            {
                float v =
                    (dp->tdir_type == TIFF_FLOAT ? TIFFFetchFloat(tif, dp)
                               ...
                               uint32_t **lpp)
{
     ...
     */
    if (*lpp == NULL &&
        (*lpp = (uint32_t *)CheckMalloc(tif, nstrips * sizeof(uint32_t),
    ...
    if (dir->tdir_type == (int)TIFF_SHORT)
    {
         ...
         */
        uint16_t *dp = (uint16_t *)CheckMalloc(
            tif, dir->tdir_count * sizeof(uint16_t), "to fetch strip tag");
        if (dp == NULL)
            return (0);
...
static void ChopUpSingleUncompressedStrip(TIFF *tif)
{
    ...
    newcounts =
        (uint32_t *)CheckMalloc(tif, nstrips * sizeof(uint32_t),
